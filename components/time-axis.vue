	...
	<view class="box">
		...
		<view class="set-1">
			<view class="set-2" :key="index" v-for="(item,index) in options">
				<text class="set2-test">{{item.createtime | formatTime}}</text>
				<view class="set-view">
					<view class="set-view-test">
						<view class="set-view-test1">{{item.ucode}}</view>
						<view class="set-view-test2">{{item.content}}</view>
					</view>
					<!-- <view class="set-view-test3"></view> -->
					<view class="set-view-test4">
						<view class="set-zuo">外设名称：{{item.dname}}</view>
