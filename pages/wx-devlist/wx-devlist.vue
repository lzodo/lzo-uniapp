<template>
	<view class="wrap" style="width: 100%">
		<view class="header-search">
			<u-search placeholder="请输入关键字" v-model="keyword" @input="
                    clear();
                    number = 1;
                    getList();
                " bg-color="#fff" input-align="center" height="70"></u-search>
		</view>
					...
					<view class="demo-price">
						...
						<view class="demo-status">
							<span style="color: #5eea62" v-show="item.connstate == '在线'"
								class="value">{{ item.connstate }}</span>
...
<script>
	export default {
		...
		methods: {
			refreshToken() {
				let that = this;
				this.AxiosGET("/securityApi/auth/token/refresh", {})
					.then((data) => {
						if (data.status == 0) {
							console.log(data);
							uni.setStorageSync("loginInfoJson",JSON.stringify(data.result));
							var expires = data.result.expires;
							setTimeout(this.refreshToken, (expires - 180) * 1000);
						}
					});
			},
			godevinfo(item) {
			...
			getList(state) {
				...
				this.AxiosPOST("/currencyApi/page/currency/showDeviceBy", params).then((res) => {
					if (res.status == 0) {
						...
						if (state == "concat") {
							this.devlist = this.devlist.concat(this.getIco(res.result.list, this.$store.state
								.devModelList));
						} else {
